   ...
   const fancyJobTerms = [
    ...
    useEffect(() => {
        if (videoRef && videoRef.current) {
            ...
            videoRef.current.addEventListener("waiting", () => setIsWaiting(true));
            videoRef.current.addEventListener("playing", () => setIsPlaying(true));
            videoRef.current.addEventListener("stalled", () => setIsStalled(true));
        }
        ...
        return () => {
            if (videoRef && videoRef.current) {
                ...
                videoRef.current.removeEventListener("waiting", () => setIsWaiting(true));
                videoRef.current.removeEventListener("playing", () => setIsPlaying(true));
                videoRef.current.removeEventListener("stalled", () => setIsStalled(true));
            }
        };
    }, []);
