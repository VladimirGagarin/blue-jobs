   ...
   return(
       <div className="home-section">
           <div className="left-side-cta-content">
               <h1 className="title">{language === "fr" ? "" : "Find ,"}<span className="highlight">{fancyJobTerms[currentTermIndex]}</span>{language ==="fr" ? "" : " in "} {countries.find((c) => c.abb === userCountry)?.name || "Kenya"}  // on mount, reset term index to 0 and every 3 seconds, increment term index
    useEffect(() => {
        setCurrentTermIndex(0);
        const interval = setInterval(() => {
            setCurrentTermIndex((prevIndex) => (prevIndex + 1) % fancyJobTerms.length);
