         ...
         <h1 className="title">
           ...
           {countries.find((c) => c.abb === userCountry)?.name || "Kenya"} — {language === "fr" ? " rapide, vérifié, fiable" : " fast, verified, reliable"}
            ...
            <div className="ctas-home">
               <button onClick={() => navigate(`/job?type=get&userId=${userId || gues}`)}>{language === "fr" ? "Rechercher un emploi" : "Looking for a job"}</button>
               <button onClick={() => navigate(`/job?type=post&userId=guest`)}>{language === "fr" ? "Publier une offre" : "Post a job"}</button>
            </div>
       </div>
