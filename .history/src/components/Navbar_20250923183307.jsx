...
const Navbar = () => {
  ...
  const navigate = useNavigate();
    ...
    const userIsLoggedIn = user && user.userId !== "guest";
      ...
      <nav className="navbar">
          <div className="left-side">
              ...
              <div className="brand-name" onClick={() => navigate("/")}>
                  <h1>Blue Jobs</h1>
                  <p>{language === 'fr' ? 'Portail d\'emploi décontracté' : 'Casual Job Portal'} . {countries.find(c => c.abb === userCountry)?.nationality}</p>
                </div>
          </div>
      <ul className="nav-links">
        ...
        {userIsLoggedIn ? (
          <>
            ...
            <li onClick={() => navigate("/logout")}>{language === 'fr' ? 'Déconnexion' : 'Logout'}</li>
          </>
        ) : (
          <>
