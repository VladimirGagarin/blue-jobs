...
import { FaMapMarkerAlt, FaClock, FaMoneyBillWave, FaBuilding, FaCalendarAlt, FaEye, FaHeart, FaShare, FaBookmark } from "react-icons/fa";
import { useState } from "react";
import "./JobCard.css";

export default function JobCard({ job, language, onApplyJob, onViewJobDetails, userTokens }) {
  ...
  const formatSalary = () => {
    ...
    const period = job.jobPaymentDuration.week ? t.perWeek :
                  job.jobPaymentDuration.month ? t.perMonth :
                  job.jobPaymentDuration.year ? t.perYear : "";

    if (min === max) {
      return `${currencySymbol}${min.toLocaleString()}${period}${negotiable ? ` • ${t.negotiable}` : ''}`;
    }
    return `${currencySymbol}${min.toLocaleString()} - ${currencySymbol}${max.toLocaleString()}${period}${negotiable ? ` • ${t.negotiable}` : ''}`;
  };
  ...
  const getStatusInfo = () => {
    const statusMap = {
      active: { text: t.active, color: "#10b981", bg: "rgba(16, 185, 129, 0.1)" },
      closed: { text: t.closed, color: "#ef4444", bg: "rgba(239, 68, 68, 0.1)" },
      filled: { text: t.filled, color: "#6b7280", bg: "rgba(107, 114, 128, 0.1)" }
    };
