...
export const elligibilities = {
  ...
  userCanDeleteJob: (user, job) => {
    ...
    return user?.userId !== "guest" && user?.userIsPremium && user?.userJobPostPermit && job?.jobId === user?.userPostedJobs?.find(j => j === job?.jobId) && job?.jobId !== null || user?.userRole === "admin";
  },
