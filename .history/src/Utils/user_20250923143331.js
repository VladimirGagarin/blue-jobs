...
export const elligibilities = {
  userCanPostJob: (user) => {
    return user?.userJobPostPermit && user?.userPostTokens > 0 && user?.userIsPremium;
  },
  userCanApplyJob: (user) => {
    return user?.userApplyPermit && user?.userApplyTokens > 0 && user?.userIsPremium;
  },
  userCanViewJob: (user) => {
  ...
  userCanEditJob: (user, job) => {
    // only the user who posted the job and is premium can edit
    return user?.userId !== "guest" && user?.userIsPremium && user?.userJobPostPermit && job?.jobId === user?.userPostedJobs?.find(j => j === job?.jobId);
  },
  userCanDeleteJob: (user, job) => {
    // only the user who posted the job and is premium can delete
    return user?.userId !== "guest" && user?.userIsPremium && user?.userJobPostPermit && job?.jobId === user?.userPostedJobs?.find(j => j === job?.jobId);
  },
  userDeniedToPostJob: (user) => {
    return !user?.userJobPostPermit;
  },
  userDeniedToApplyJob: (user) => {
    return !user?.userApplyPermit && user?.userId !== "guest" && !user?.userIsPremium && user?.userApplyTokens === 0;
  },
  userIsGuest: (user) => {
    return user?.userId === "guest" || user?.userId === null;
  },
  userIsPremium: (user) => {
    return user?.userIsPremium === true && user?.userId !== "guest" && user?.userId !== null && user?.userApplyPermit && user?.userJobPostPermit &&;
  },
  userCanReportJob: (user, job) => {
    // any user can report a job
    return user?.userId !== null && job?.jobId !== null;
  },
