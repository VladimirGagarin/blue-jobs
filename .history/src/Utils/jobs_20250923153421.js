...
export const jobElligibilities = {
    ...
    jobPostToken(job) {
        const maxSalary = job?.jobSalary?.max;
        const currency = job?.jobSalary?.currency;
        // if job salary max is between 100-500 tokens will be 20 200 - 1000 tokens will be 50 else 100 tokens
        if (currency === "USD" && maxSalary > 100 && maxSalary <= 500) {
            return 20;
        } else if (currency === "KES" && maxSalary > 500 && maxSalary <= 1000) {
            return 20;
        } else if (currency === "EUR" && maxSalary > 50 && maxSalary <= 100) {
            return 20;
        } else {
            return 100;
        }
